...
ATLAS_RUNTIME_ENV="$AUTH_DIR/atlas-runtime.env"
...
if [[ "$QUAY_ENABLED" -eq 1 && "$QUAY_SERVE_SUPPORTED" -eq 1 && -f "$QUAY_SERVE_SRC" ]]; then
  PYTHONPATH="$FORK_DIR/installer" "$PYTHON_BIN" -m hermes_installer \
    ...
    --service-name quay-serve \
    --out /etc/default/quay-serve
  ...
  if command -v systemctl >/dev/null 2>&1; then
    ...
    PYTHONPATH="$FORK_DIR/installer" "$PYTHON_BIN" -m hermes_installer \
      ...
      --out /etc/systemd/system/quay-serve.service \
      --set "AGENT_USER=$AGENT_USER" \
      --set "TARGET_DIR=$TARGET_DIR"
    systemctl daemon-reload
...
if [[ "$QUAY_ENABLED" -eq 1 && "$QUAY_SERVE_SUPPORTED" -eq 1 && -n "$QUAY_ADMIN_PUBLIC_BASE_URL" && -f "$HERMES_DASHBOARD_SRC" ]]; then
  PYTHONPATH="$FORK_DIR/installer" "$PYTHON_BIN" -m hermes_installer \
    ...
    --out /etc/default/hermes-dashboard

  if command -v systemctl >/dev/null 2>&1; then
    echo "==> installing systemd service for hermes-dashboard (user=$AGENT_USER, bind=127.0.0.1:9119)"
    PYTHONPATH="$FORK_DIR/installer" "$PYTHON_BIN" -m hermes_installer \
      render-systemd-unit \
      --template "$HERMES_DASHBOARD_SRC" \
      --out /etc/systemd/system/hermes-dashboard.service \
      --set "AGENT_USER=$AGENT_USER" \
