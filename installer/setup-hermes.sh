...
QUAY_ADMIN_PUBLIC_BASE_URL="$(python3 "$VALUES_HELPER" --values "$VALUES_FILE" get quay.admin.public_base_url 2>/dev/null || true)"
QUAY_ADMIN_PUBLIC_BASE_URL="${QUAY_ADMIN_PUBLIC_BASE_URL%/}"

if [[ -z "$QUAY_VERSION" ]]; then
  echo "==> quay.version unset in $VALUES_FILE; skipping quay provisioning"
else
  QUAY_ENABLED=1
  QUAY_ORCHESTRATOR_ENABLED_RAW="$(python3 "$VALUES_HELPER" --values "$VALUES_FILE" get quay.orchestrator.enabled 2>/dev/null || true)"
  if [[ "$QUAY_ORCHESTRATOR_ENABLED_RAW" == "true" ]]; then
    QUAY_ORCHESTRATOR_ENABLED=1
  fi
  case "$(uname -m)" in
...
# usable on first start. When the hosted Quay Admin path is configured, [web]
# pulls in fastapi + uvicorn for hermes-dashboard.service; without it the
# service fails immediately after setup.
HERMES_INSTALL_EXTRAS="slack"
if [[ "$QUAY_ENABLED" -eq 1 && "$QUAY_SERVE_SUPPORTED" -eq 1 && -n "$QUAY_ADMIN_PUBLIC_BASE_URL" ]]; then
  HERMES_INSTALL_EXTRAS="slack,web"
fi
"$TARGET_DIR/hermes-agent/venv/bin/pip" install --quiet -e "$TARGET_DIR/hermes-agent[$HERMES_INSTALL_EXTRAS]"
...
if [[ "$QUAY_ENABLED" -eq 1 ]]; then
  ...
  QUAY_ORCHESTRATOR_CONFIG_OUT="$TARGET_DIR/quay/orchestrator.json"
  echo "==> rendering $QUAY_ORCHESTRATOR_CONFIG_OUT from $VALUES_FILE"
  python3 "$VALUES_HELPER" --values "$VALUES_FILE" \
    render-quay-orchestrator-config --out "$QUAY_ORCHESTRATOR_CONFIG_OUT" --force
  chown "$AGENT_USER:$AGENT_USER" "$QUAY_ORCHESTRATOR_CONFIG_OUT"
...
# ---------- hermes-dashboard ----------
# Long-running loopback Hermes dashboard. When quay.admin.public_base_url is
# configured, Slack /quay-admin links point at this dashboard's Quay Admin
# proxy path. The edge proxy must still be configured outside this installer.

HERMES_DASHBOARD_SRC="$OPS_DIR/hermes-dashboard.service"

if [[ "$QUAY_ENABLED" -eq 1 && "$QUAY_SERVE_SUPPORTED" -eq 1 && -n "$QUAY_ADMIN_PUBLIC_BASE_URL" && -f "$HERMES_DASHBOARD_SRC" ]]; then
  install -d -o root -g root -m 0755 /etc/default
  if [[ -f /etc/default/hermes-dashboard ]]; then
    echo "==> /etc/default/hermes-dashboard already present (preserving)"
  else
    echo "==> seeding /etc/default/hermes-dashboard"
    cat >/etc/default/hermes-dashboard <<'EOF'
# Generated by setup-hermes.sh — extra env for hermes-dashboard.service.
# Edits here survive re-runs of the installer. Keep QUAY_ADMIN_TOKEN in
# <HERMES_HOME>/auth/quay.env; setup-hermes.sh generates and preserves it.
EOF
    chown root:root /etc/default/hermes-dashboard
    chmod 0644 /etc/default/hermes-dashboard
  fi

  if command -v systemctl >/dev/null 2>&1; then
    echo "==> installing systemd service for hermes-dashboard (user=$AGENT_USER, bind=127.0.0.1:9119)"
    sed -e "s|__AGENT_USER__|$AGENT_USER|g" \
        -e "s|__AGENT_HOME__|$AGENT_HOME|g" \
        -e "s|__TARGET_DIR__|$TARGET_DIR|g" \
        "$HERMES_DASHBOARD_SRC" \
      | install -o root -g root -m 0644 /dev/stdin /etc/systemd/system/hermes-dashboard.service
    systemctl daemon-reload
    ...
    for _ in {1..20}; do
      if systemctl is-active --quiet hermes-dashboard.service; then
        dashboard_code="$(curl -sS -o /dev/null -w "%{http_code}" \
          http://127.0.0.1:9119/quay/admin/ 2>/dev/null || true)"
    ...
    if [[ "$dashboard_ready" -ne 1 ]]; then
      echo "FAIL: hermes-dashboard.service did not return HTTP 401 at http://127.0.0.1:9119/quay/admin/" >&2
      systemctl --no-pager --lines=80 status hermes-dashboard.service >&2 || true
      journalctl -u hermes-dashboard.service -n 100 --no-pager >&2 || true
      exit 1
    fi
  else
    echo "==> systemctl not present; skipping hermes-dashboard service enable" >&2
  fi
elif [[ "$QUAY_ENABLED" -eq 1 && "$QUAY_SERVE_SUPPORTED" -eq 1 && -n "$QUAY_ADMIN_PUBLIC_BASE_URL" ]]; then
  echo "==> WARNING: $HERMES_DASHBOARD_SRC missing; skipping hermes-dashboard install" >&2
elif [[ "$QUAY_ENABLED" -eq 1 && "$QUAY_SERVE_SUPPORTED" -eq 1 ]]; then
  echo "==> hermes-dashboard not enabled because quay.admin.public_base_url is unset"
fi
...
# deployment opts into orchestrator Slack posting and reply polling.

QUAY_ORCH_SERVICE_SRC="$OPS_DIR/quay-orchestrator.service"
QUAY_ORCH_TIMER_SRC="$OPS_DIR/quay-orchestrator.timer"
QUAY_ORCH_RUNNER_SRC="$OPS_DIR/quay-orchestrator-runner"
QUAY_ORCH_RUNNER_DST="/usr/local/sbin/quay-orchestrator-runner"

if command -v systemctl >/dev/null 2>&1; then
  echo "==> disabling legacy brix-orchestrator units if present"
  systemctl disable --now brix-orchestrator.timer brix-orchestrator.service >/dev/null 2>&1 || true
  rm -f /etc/systemd/system/brix-orchestrator.timer /etc/systemd/system/brix-orchestrator.service
  systemctl daemon-reload
fi
...
if [[ "$QUAY_ENABLED" -eq 1 && "$QUAY_ORCHESTRATOR_ENABLED" -eq 1 && -f "$QUAY_ORCH_SERVICE_SRC" && -f "$QUAY_ORCH_TIMER_SRC" && -f "$QUAY_ORCH_RUNNER_SRC" ]]; then
  ...
  if [[ -f /etc/default/quay-orchestrator ]]; then
    echo "==> /etc/default/quay-orchestrator already present (preserving)"
  elif [[ -f /etc/default/brix-orchestrator ]]; then
    echo "==> migrating /etc/default/brix-orchestrator to /etc/default/quay-orchestrator"
    install -o root -g root -m 0644 /etc/default/brix-orchestrator /etc/default/quay-orchestrator
  else
    echo "==> seeding /etc/default/quay-orchestrator"
    cat >/etc/default/quay-orchestrator <<'EOF'
# Generated by setup-hermes.sh - overrides for quay-orchestrator.service.
...
EOF
  ...
  if command -v systemctl >/dev/null 2>&1; then
    ...
    sed -e "s|__AGENT_USER__|$AGENT_USER|g" \
        -e "s|__TARGET_DIR__|$TARGET_DIR|g" \
        "$QUAY_ORCH_SERVICE_SRC" \
      | install -o root -g root -m 0644 /dev/stdin /etc/systemd/system/quay-orchestrator.service
    install -o root -g root -m 0644 \
      "$QUAY_ORCH_TIMER_SRC" /etc/systemd/system/quay-orchestrator.timer
    systemctl daemon-reload
    systemctl enable --now quay-orchestrator.timer
  else
    echo "==> systemctl not present; skipping quay-orchestrator timer enable" >&2
  fi
elif [[ "$QUAY_ENABLED" -eq 1 && "$QUAY_ORCHESTRATOR_ENABLED" -eq 1 ]]; then
  [[ -f "$QUAY_ORCH_SERVICE_SRC" ]] \
    || echo "==> WARNING: $QUAY_ORCH_SERVICE_SRC missing; skipping quay-orchestrator install" >&2
  [[ -f "$QUAY_ORCH_TIMER_SRC" ]] \
    || echo "==> WARNING: $QUAY_ORCH_TIMER_SRC missing; skipping quay-orchestrator install" >&2
  [[ -f "$QUAY_ORCH_RUNNER_SRC" ]] \
