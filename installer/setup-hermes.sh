...
# ---------- hermes-gateway ----------
# Long-running messaging gateway (Slack via Socket Mode today; Telegram /
# Discord / etc. when their tokens land in the env).
#
# Why we don't ship a static unit file:
#   `hermes gateway install --system` (in hermes_cli/gateway.py) is the
#   upstream-supported path and dynamically generates the unit. On every
#   `hermes update` the CLI's repair pass compares the installed unit
#   against `generate_systemd_unit()` and rewrites it on drift — a static
#   ops-side unit would get silently clobbered along with any
#   EnvironmentFile= lines we'd added.
#
# Instead, we let the CLI own the canonical unit and layer our local
# additions via a systemd drop-in:
#
#   1. /etc/systemd/system/hermes-gateway.service          ← CLI-generated
#   2. /etc/systemd/system/hermes-gateway.service.d/
#        slack-env.conf                                    ← we own this
#
# Drop-ins merge with the main unit; multiple EnvironmentFile= lines are
# additive. Future `hermes update` runs only refresh (1), leaving (2)
# untouched.
#
# Files referenced from the drop-in:
#   /etc/default/hermes-gateway     non-secret operator config (channel
#                                   allowlist, etc). 0644 root:root.
#   $TARGET/auth/slack.env          bot/app tokens. 0640 root:agent —
#                                   matches the github-app.env pattern.
#
# Enable policy: only `enable --now` if slack.env is staged, otherwise
# leave the unit installed-but-disabled to avoid a flap-restart loop
# against missing tokens during initial provisioning.
...
# Generated by setup-hermes.sh — overrides for hermes-gateway.service.
# Edits here survive re-runs of the installer; delete this file to force
# regeneration from defaults.
#
# SLACK_ALLOWED_CHANNELS: comma-separated channel IDs the bot will respond
# in. When unset the bot responds anywhere it's invited (mention or DM).
# Use "*" to disable the gate explicitly. Operator-readable, so per-channel
# config and tokens stay separated.
#
# DM authorization (SLACK_ALLOWED_USERS) lives in slack.env, next to the
# tokens, so user-id changes stay adjacent to credentials in the 0640 file.
...
EOF
  ...
  if command -v systemctl >/dev/null 2>&1; then
    # Step 1: install the canonical unit via the upstream CLI. HERMES_HOME
    # override pins the unit's HERMES_HOME to our $TARGET (root would
    # otherwise resolve it to /root/.hermes and the post-install remap
    # only handles default ~/.hermes paths). --force makes the call
    # idempotent — re-runs of the installer rewrite the unit cleanly.
    ...
    # Step 2: drop our slack-env additions next to the main unit. Path is
    # sed-templated so non-default --target installs (e.g. /opt/hermes)
    # point at the right slack.env. The drop-in itself is root-owned
    # because it lives under /etc/systemd/system/ — the agent reads it
    # only via systemd's environment expansion.
