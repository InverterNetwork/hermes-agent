...
# Discord / etc. when their tokens land in the env). The unit is templated
# from ops/hermes-gateway.service with __AGENT_USER__ and __TARGET_DIR__
# substituted at install time so non-default --user / --target installs work.
#
# Tokens live in $TARGET/auth/slack.env (mode 0640 root:agent), hand-edited
# during initial provisioning (parallel to the github-app.env pattern). The
# unit references the file with a leading `-` so a missing file degrades to
# "unit loads, gateway fails to enable Slack" rather than a load-time error.
#
# Operator config (channel allowlist, user allowlist, etc.) lives in
# /etc/default/hermes-gateway and is preserved across re-runs so post-install
# hand-edits survive future installer invocations (matches hermes-sync).
#
# Enable policy: only `enable --now` if slack.env is staged, otherwise drop
# the unit but leave it disabled with a clear message — first-install should
# not flap-restart with a missing-token error before tokens arrive.

GATEWAY_UNIT_SRC="$OPS_DIR/hermes-gateway.service"
GATEWAY_UNIT_DST="/etc/systemd/system/hermes-gateway.service"
...
# regeneration from defaults.
HOME=$AGENT_HOME
HERMES_HOME=$TARGET_DIR
# SLACK_ALLOWED_CHANNELS: comma-separated channel IDs the bot will respond
# in. When unset the gateway responds anywhere it's invited (#mention or
# DM). Use "*" to disable the gate explicitly. Leave SLACK_ALLOWED_USERS in
# slack.env so adding a Slack user-id stays adjacent to the tokens.
...
EOF
  ...
  if command -v systemctl >/dev/null 2>&1; then
    echo "==> installing systemd unit for hermes-gateway (user=$AGENT_USER)"
    sed -e "s|__AGENT_USER__|$AGENT_USER|g" \
        -e "s|__TARGET_DIR__|$TARGET_DIR|g" \
        "$GATEWAY_UNIT_SRC" \
      | install -o root -g root -m 0644 /dev/stdin "$GATEWAY_UNIT_DST"
    systemctl daemon-reload
    # Only enable when tokens are staged. Without slack.env the gateway
    # would flap on Restart=on-failure trying to connect with no token —
    # noisy in journalctl and confusing for an operator mid-install.
    ...
    echo "==> systemctl not present; skipping hermes-gateway enable (unit installed)" >&2
  fi
else
  echo "==> WARNING: $GATEWAY_UNIT_SRC missing; skipping hermes-gateway install" >&2
fi
