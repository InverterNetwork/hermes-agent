...
if [[ -f "$CODE_SYNC_SCRIPT_SRC" ]]; then
  ...
  if [[ -f /etc/default/hermes-code-sync ]]; then
    ...
    # systemd's User=__AGENT_USER__ already sets HOME from the agent's
    # passwd entry, so we don't seed it here. The values-file + helper
    # paths are pinned so the script reads the same source of truth the
    # installer was driven from, even if the operator later runs the
    # installer with a different --values flag (next setup-hermes.sh run
    # rewrites this file iff it's been deleted).
