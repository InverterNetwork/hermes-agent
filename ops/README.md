...
| `ops/quay-orchestrator.timer`              | systemd timer unit. 1-min cadence, protected by the runner lock; the human reply wait releases the lock so delivery-only outbox drains are not held behind long handoffs. |
