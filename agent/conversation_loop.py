...
def _restore_or_build_system_prompt(agent, system_message, conversation_history):
    ...
    # prompt) — build from scratch. Snapshot the memory state before
    # caching the prompt so a snapshot/persist failure leaves the cache
    # empty and the next attempt retries the whole first-turn setup.
    new_system_prompt = agent._build_system_prompt(system_message)
    if hasattr(agent, "_record_session_start_memory_snapshot"):
...
<<<<<<< HEAD
    ...
    try:
        ...
        _set_state_ctx(agent.session_id, agent._user_turn_count)
    except Exception:
        pass
