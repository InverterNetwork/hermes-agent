...
| `SLACK_RESPONSE_POLICY` | Channel response policy: `mention_to_wake_quiet_thread` (default) or `thread_followup` |
| `SLACK_STRICT_MENTION` | Require an explicit `@mention` on every Slack channel message (default: `false`) |
...
| `QUAY_ADMIN_TOKEN` | Bearer token required by `quay serve` when the pinned Quay binary supports the Admin UI/API. The installer generates and preserves it in `<HERMES_HOME>/auth/quay.env`; the service binds to `127.0.0.1` by default. |
| `QUAY_ADMIN_ALLOWED_USERS` | Comma-separated Slack user IDs allowed to pass `POST /quay/admin/authorize` for Quay Admin access. Empty or unset denies all users; wildcard access is not supported. This allowlist is independent of `SLACK_ALLOWED_USERS`, `GATEWAY_ALLOWED_USERS`, and `GATEWAY_ALLOW_ALL_USERS`. |
