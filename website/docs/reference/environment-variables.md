...
| `QUAY_ADMIN_ALLOWED_USERS` | Comma-separated Slack user IDs allowed to request Quay Admin access. Empty or unset denies all users; wildcard access is not supported. This allowlist is independent of `SLACK_ALLOWED_USERS`, `GATEWAY_ALLOWED_USERS`, and `GATEWAY_ALLOW_ALL_USERS`. |
