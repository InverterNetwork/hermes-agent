...
3. Run `installer/setup-hermes.sh` on the target host. The installer:
   ...
   - generates and preserves `<HERMES_HOME>/auth/quay.env`'s `QUAY_ADMIN_TOKEN` when `quay.version` is set, then installs `quay-serve.service` bound to `127.0.0.1:9731`,
