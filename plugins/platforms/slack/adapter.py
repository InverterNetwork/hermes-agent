...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # slash triggers (from SKILL.md frontmatter, e.g. /take) are
            # included too. A single regex matcher dispatches all of them
            # to one handler so we don't need N identical @app.command()
            # decorators — slack_bolt would otherwise treat unmatched
            # slashes as "Unhandled request" and never ack, breaching the
            # 3-second Slack deadline and surfacing "did not respond" to
            # the user.
            #
            # The slash commands must ALSO be declared in the Slack app
            # manifest (see installer/values_helper.py render-manifest, which
            # builds features.slash_commands from the same union — native
            # + skill triggers — so the runtime and manifest can't drift).
            # In Socket Mode, Slack routes the command event through the
            # socket regardless of the manifest's request URL, but it will
            # not deliver an event for a slash command the manifest
            # doesn't declare.
            #
...
<<<<<<< HEAD:gateway/platforms/slack.py
        media_urls, media_types, attachment_notices, inline_text = (
            await self._extract_slack_media(
                event, channel_id=channel_id, team_id=team_id,
            )
...
>>>>>>> upstream/main:plugins/platforms/slack/adapter.py
    ...
    def _slack_thread_followup_is_actionable(
        ...
        *,
        ...
        request_re = re.compile(
            ...
            r"question|thoughts|wdyt|what|why|how|when|where|which"
            r")\b",
            re.IGNORECASE,
        )
